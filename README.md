...
- **Análisis Híbrido Avanzado (DAST/SAST)**: Motor profundo de Inyección Dinámica (DAST) capaz de extraer Formularios DOM e inyectar *Payloads* destructivos automatizadamente sobre cuerpos POST y JSON de Single Page Applications (SPAs) y APIs REST, complementado con SAST (estudio estático de fugas de secretos JS).
...
5. **Araña DAST Profunda (POST/Form Payload Dispatcher)**: El escáner ya no se limita a probar parámetros GET superficiales. El motor central intercepta y destripa el HTML buscando etiquetas `<form>`, recolectando mecánicamente sus campos (`<input>`, `<select>`), rutas (`action`) y método HTTP. Luego, ensambla diccionarios de variables y los dispara de forma concurrente hacia las Interfaces de los Plugins Ofensivos (`sqli.py`, `xss.py`), orquestando inyecciones automatizadas contra portales de autenticación complejos y APIs REST.
