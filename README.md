...
1. **Doble Escudo Heurístico (Soft404Profiler + SensitiveFileValidator)**: Antes de despachar el enjambre de hilos asíncronos en el `crawl_loop`, el motor ejecuta una calibración síncrona enviada a un sub-hilo independiente (`asyncio.to_thread`) para perfilar cómo responde el objetivo a archivos inexistentes. **Silenciamiento Exhaustivo**: El módulo enmudece limpiamente dependencias pre-cargadas en SO ofensivos (ej. *urllib3 RequestsDependencyWarning*). Luego, durante la ejecución asíncrona, cualquier hallazgo `200 OK` pasa por el Validador de Capa 7 (`SensitiveFileValidator`). Este módulo rechaza agresivamente Falsos Positivos usando **Tolerancia Volumétrica Dinámica (25%)** adaptándose a rutas escupidas en plantillas de error, y buscando colisiones contra firmas JSON nativas (`{"error": "not found"}`) para aniquilar "Soft 404s" invisibles antes del reporte.
...
8. **Seguridad Ofensiva Cero-Defectos y Profiling (OOM-Safe)**: La arquitectura está estandarizada por certificaciones de calidad estrictas Linter (`flake8` Zero-Defects) previniendo dependencias muertas y fallas de variables a nivel intérprete. Adicionalmente, el Core incorpora un motor lógico de **Mutación Context-Aware**, el cual evalúa si un disparo asíncrono ataca un formulario web convencional (`data=...`) o una API Rest nativa (`json=payload`), inhibiendo codificaciones estáticas destructivas (como *URL-Encoding*) sobre cuerpos JSON que causarían fallos silenciosos por denegación de inyección. A nivel defensivo, el motor implementa *Barricadas de Bytes L7* limitando dinámicamente descargas a `5MB` por Request, previniendo colapsos de memoria RAM por Tarpitting malicioso.
